...
class AudioDecoder:
    def __init__(self, config_path, flow_ckpt_path, hift_ckpt_path, device="cuda"):
        ...
        self.sampling_rate = self.scratch_configs['sample_rate']
        ...
        self.mel_overlap_len = int(self.token_overlap_len / self.flow.input_frame_rate * self.sampling_rate / 256)
    ...
    def offline_inference(self, token, embedding=torch.zeros(1, 192), return_mel=False):
        this_uuid = str(uuid.uuid1())
        tts_speech, tts_mel = self.token2wav(token, uuid=this_uuid, embedding=embedding, finalize=True)
        output = (tts_speech.cpu(), self.sampling_rate)
        if return_mel:
            output += (tts_mel.cpu(),)
        return output
