...
| 2026-05-17 | 100-clip degradation scale-up config prepared | pending | `experiments/configs/degradation_full_100.yaml` | Config only | Same 13 corruptions as `degradation_full.yaml`, scaled from 50 to 100 clips per language. | This is the clean next robustness scale-up: same checkpoint, seed, corruptions, and per-language split, only larger sample count. | Next command: `/data/venv/bin/python experiments/run_experiment.py --config experiments/configs/degradation_full_100.yaml --run-id degradation_full_100_20260517`. |
